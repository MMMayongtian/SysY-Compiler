...
bool LinearScan::linearScanRegisterAllocation() {
    ...
    for (int i = 0; i < intervals.size(); i++){
        ...
        // unhandled interval 的 interval 并回收可用寄存器
...
void LinearScan::modifyCode() {
    ...
    for(auto block : func->getBlocks()){
        auto inst_last = block->getInsts().back();
        if(inst_last->isBX()){
            // 恢复栈空间 ADD指令 sp移动
            auto sp = new MachineOperand(MachineOperand::REG, 13); // sp
            auto offset = new MachineOperand(MachineOperand::IMM, func->getStackSize());
            if(abs(func->getStackSize())>255){
                auto r1 = new MachineOperand(MachineOperand::REG, 1); // sp
                auto load = new LoadMInstruction(nullptr,r1, offset);
                inst_last->insertBefore(load);
                offset = new MachineOperand(*r1);
            }
            auto spAdd = new BinaryMInstruction(block, BinaryMInstruction::ADD, sp, sp, offset);
            inst_last->insertBefore(spAdd);

            // BX 带状态切换跳转指令 保存 fp 和 lr 寄存器
            auto fp = new MachineOperand(MachineOperand::REG, 11); // fp
            auto lr = new MachineOperand(MachineOperand::REG, 14); // lr
            auto stackPop = new StackMInstrcution(block, StackMInstrcution::POP, func->getSavedRegs(), fp, lr);// pop 保存状态
            inst_last->insertBefore(stackPop);
        }
    }    
}
...
void LinearScan::expireOldIntervals(Interval* interval) {
    ...
    while(active.size()!=0){
        if(active.front()->end >= interval->start){
            sort(regs.begin(),regs.end());
            return ;
        }
        regs.push_back(active.front()->rreg);
